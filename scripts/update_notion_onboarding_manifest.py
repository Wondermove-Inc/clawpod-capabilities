...
flags={'outputRoot':'--output-root','session':'--session','stateName':'--state-name','authMode':'--auth-mode','workspace':'--workspace','roots':'--roots','capabilities':'--capabilities','expectedRevision':'--expected-revision','approveHandoffs':'--approve-handoffs','sessionTimeout':'--session-timeout','now':'--now'}
def cmd(name,safety,props,required):
 return {'description':name+' secret-free Notion onboarding contract.','baseArgv':[name],'safetyClasses':safety,'inputSchema':{'type':'object','required':required,'properties':props,'additionalProperties':False},'outputSchema':out,'argMap':[{'arg':k,'type':'option','flag':flags[k],'valueType':'string' if k=='roots' else ('integer' if v.get('type')=='integer' else 'string'),'optional':k not in required} for k,v in props.items()]}
name={'type':'string','pattern':'^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$'};common={'outputRoot':{'type':'string','minLength':1,'maxLength':4096},'session':name,'stateName':name};plan={'authMode':{'type':'string','enum':['internal','pat','oauth']},'workspace':{'type':'string','minLength':1,'maxLength':200},'roots':roots,'capabilities':{'type':'string','maxLength':500}}
...
commands={'onboard.plan':cmd('onboard.plan',['readOnly'],plan,[]),'onboard.desktop.plan':cmd('onboard.desktop.plan',['readOnly'],plan,[]),'onboard.desktop.task':cmd('onboard.desktop.task',['readOnly'],plan,[]),'onboard.start':cmd('onboard.start',['externalSideEffect'],start,['outputRoot','session','workspace']),'onboard.status':cmd('onboard.status',['readOnly'],common,['outputRoot','session']),'onboard.inspect':cmd('onboard.inspect',['readOnly'],common,['outputRoot','session']),'onboard.resume':cmd('onboard.resume',['externalSideEffect','secretUse','authReuse'],resume,['outputRoot','session','expectedRevision']),'onboard.cancel':cmd('onboard.cancel',['writeSafe'],{**common,'expectedRevision':{'type':'integer','minimum':0},'now':{'type':'integer','minimum':1}},['outputRoot','session','expectedRevision'])}
