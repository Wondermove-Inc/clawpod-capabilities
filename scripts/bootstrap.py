...
def load_entries() -> dict[str, dict[str, Any]]:
    ...
    entries = {entry["id"]: entry for entry in data.get("capabilities", [])}
    missing = [capability_id for capability_id in BOOTSTRAP_IDS if capability_id not in entries]
    if missing:
...
def install_one(
    ...
    *,
    ...
    if destination_matches(destination, files):
        return {"id": entry["id"], "status": "already-installed", "destination": str(destination)}
    ...
    if dry_run:
        return {"id": entry["id"], "status": "would-install", "destination": str(destination)}
