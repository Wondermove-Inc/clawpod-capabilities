...
For Debian/Ubuntu, the required baseline is `poppler-utils tesseract-ocr tesseract-ocr-kor tesseract-ocr-eng tesseract-ocr-osd`. Install missing baseline packages immediately without pausing for approval. Verify Tesseract major 5 and actual `kor`, `eng`, and `osd` availability, then persist the successful local state and re-run preflight.
