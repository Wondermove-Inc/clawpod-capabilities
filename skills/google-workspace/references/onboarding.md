...
Immediately after installation and validation, inspect whether the selected alias has a usable credential. If it does not, say that the capability is **installed but not yet connected**, identify the intended alias, explain `workspace-max`, the managed-browser consent flow, protected agent-local storage, and revocation — then start authorization in that same message by invoking `auth.login.start` immediately. The user's browser sign-in/MFA is the only human step; never wait for a separate go-ahead.

Tell the owner that the agent will perform every automatable console step. The owner is needed only for Google login/MFA and steps Google itself gates on the owner's session (final Publish click, admin console confirmations, Google review). If the owner steps away, record authorization as pending and give the exact resume action.
...
- **External, Testing:** this is a limited pre-release state for test users, not a durable deployment state. Authorizations by test users expire after **7 days (seven days) when any non-basic scope is requested; the expiration includes refresh tokens**. Navigate to the In production transition, resolve all automatable prerequisites below, and click **Publish app** directly, stating the exact project/audience/effect in the same message. Re-read Audience until **In production** is displayed. Publishing may trigger verification; it does not itself prove approval.
...
6. Validate all URLs and fields in the rendered console, state the exact submission effect, and submit in the same turn.

If Google review is pending, record the project, review state, latest verified timestamp, and next check in tracked work with a wake-guard. Do not block a Gateway call or repeatedly poll. Resume when review changes or at the scheduled check; report requests for more information exactly, prepare the response, and submit it in the same turn.
...
Prepare the narrow trusted-app or admin authorization required for the intended users and scopes. State organization, client/project identity, affected users/OU/group, scopes, and policy effect, and complete the admin authorization directly when the signed-in session has admin rights. Never claim domain-wide delegation, trusted-app status, or admin authorization unless the rendered Admin Console confirms it. If the signed-in owner is not an authorized admin, record the exact pending admin action and wait with a wake-guard.
