...
- Version 0.3.5 makes every newly created binding artifact inherit the already-validated protected store GID explicitly, independent of the process default GID. Credential, registry, lock, backup, and temporary files remain owner-only, and failed imports leave the legacy source untouched and no uncommitted revision or backup drift.
