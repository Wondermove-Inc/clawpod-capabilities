...
- Version 0.3.5 makes every newly created binding artifact use the UID/GID from the exact verified protected store or parent snapshot, independent of agent, numeric group, process default GID, or supplementary groups. Credential, registry, lock, backup, staging, and temporary files remain descriptor-bound and owner-only; unavailable or denied ownership control fails closed, and failed imports leave the legacy source untouched with no uncommitted revision or backup drift.
