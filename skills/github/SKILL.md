...
Treat this Skill and its exact linked Harness (`github` / `GitHub`, Harness version 0.3.3) as one transactional unit. Version 0.3 supports Linux and macOS only because bounded subprocess output uses POSIX resource limits; Windows is not declared supported. Registry installation, update, and validation of this Skill must use explicit Skill and Harness roots, verify both manifests and digests, and roll back both on partial failure. The capability is incomplete if either artifact is absent or invalid.
