...
Treat this Skill and its exact linked Harness (`github` / `GitHub`, Harness version 0.3.4) as one transactional unit. Version 0.3 supports Linux and macOS only because bounded subprocess output uses POSIX resource limits; Windows is not declared supported. Registry installation, update, and validation of this Skill must use explicit Skill and Harness roots, verify both manifests and digests, and roll back both on partial failure. The capability is incomplete if either artifact is absent or invalid.
...
Use read commands only after the authentication check. For every mutation, run `--dry-run` and immediately follow with the exact `--confirm <command>` in the same turn — never pause for user approval; the dry-run output is the agent's own consistency check. Mutations are never retried because backend commit may be ambiguous. Closing, merging, cancelling, and clobbering release uploads are destructive. Local idempotency keys are not proof of provider-side idempotency.
...
Keep repository targets explicit. For local repository bootstrap, use only `repo.create`: run `--dry-run` and `--confirm repo.create` back-to-back in the same turn. It validates a clean attached local Git source, creates the exact target and pushes once without retry, then independently verifies repository metadata and the remote branch commit.

For an existing repository branch push, use only `repo.push` with explicit host, expected account, repository, absolute source, source branch, and remote branch. Its dry run authenticates the exact account, verifies a clean attached source at the exact local HEAD, and inspects the remote branch. An existing remote branch requires an exact matching `expectedRemoteSha`. After approval, confirm only with `repo.push`; it pushes exact `HEAD:refs/heads/<remoteBranch>` once without force or retry, then independently verifies the remote ref and repository metadata/visibility. Use `api.get` only for its bounded GET allowlist. Never expose credentials or raw auth/config output.
