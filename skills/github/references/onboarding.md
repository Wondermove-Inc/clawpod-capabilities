...
If verification fails, report the capability as installed but not connected. Revoke access using GitHub account settings or `gh auth logout` outside this capability. Each future mutation still runs preview and confirm, chained by the agent in one turn.
