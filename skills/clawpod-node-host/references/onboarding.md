...
| `credentials` | Accept password, key, SSH agent, or Tailscale SSH without product-level preference. A user may send the password in chat; move it immediately into protected runtime injection and discard plaintext after capture. | Ask for any supported method; do not re-echo or persist secret material after capture. |
