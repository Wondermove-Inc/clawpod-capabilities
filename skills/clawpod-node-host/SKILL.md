...
4. Obtain the requesting user's actual Gateway connection values using the authorized local tools and effective runtime sources described in [onboarding](references/onboarding.md). Alongside the installer link, provide the verified **Gateway host's Tailscale DNS name/IP**, complete **root WebSocket URL with scheme and port**, and **actual active Gateway token** as separate labeled values. In password mode, provide the active password instead. Never substitute a redacted value, SecretRef, example, or another host's address. Keep credentials out of URLs, public repository files, and diagnostic logs. Have the user open **ClawPod Node**, copy the URL into the Gateway address field, enter a display name, select the matching authentication mode, and copy the supplied credential into its separate local field. If a value cannot be verified or Tailscale is unavailable, report the specific missing value or route and use an existing reachable endpoint when available; do not fabricate connection details.
