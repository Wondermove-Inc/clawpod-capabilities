...
2. Key, SSH agent, and Tailscale SSH are supported alongside password. A password is only usable when it can be entered through a **protected channel** (runtime secret injection / an env var the harness reads) — never typed into chat, a plan, a log, or any response. **If the only input channel is chat, do not offer or recommend the password method** (a chat message cannot carry a secret safely); prefer key, SSH agent, or Tailscale SSH, which need no secret in chat. If a password does arrive through a protected channel, immediately place it in protected runtime injection and discard the captured plaintext. Pass only `password-env:NAME`, `key-env:NAME`, `agent`, or `tailscale` references—never secret values in argv, files, state, scripts, plans, logs, or responses. These are **SSH bootstrap credentials** for the agent's one-time SSH to the node — not the ClawPod Gateway's authentication. Keep the Gateway in **token-based connection mode**; never tell the user to switch the Gateway to password mode. The node connects through the Gateway token and the pairing approval, not a Gateway password.
