...
4. The approved remote script installs exactly `openclaw@2026.4.11`, verifies that exact version, installs and starts the node with `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1` set (required for the node-to-Gateway private WebSocket connection — without it the node cannot connect), and checks pairing readiness. Continue through pair and verify; resume at the first incomplete idempotent stage.
