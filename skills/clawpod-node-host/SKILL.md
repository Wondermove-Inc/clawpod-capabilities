...
- Before any mutation in the SSH-driven fallback, state the exact ClawPod-visible effect and apply with the fresh plan's confirmation in the same turn — never pause for user approval. The self-service track has no approvals at all: the user runs the script themselves and the pairing approval is the agent's own act on its own pairing queue.
...
3. Run bounded noninteractive preflight, then generate `bootstrap.plan` and chain `bootstrap.apply` with the plan's exact confirmation in the same turn; resume its idempotent preflight/upload/execute/verify stages from the first incomplete stage. On partial effects, provide one retry or revoke/rollback action.
...
5. Generate the matching lifecycle plan, apply it with its exact confirmation in the same turn, and verify observed postconditions. Preserve `node.json`, pairing records, exec approvals, and browser policy during uninstall/rollback.
