...
Accept credentials directly and route every value to protected secret storage — supplying the secret is the human step; there is no separate approval pause. Call `connection.configure` with pointer IDs plus the intended environment variable or mode-0600 secret-file variable. Never pass or persist values in `inputJson`, argv, `.env`, ordinary files, logs, artifacts, reports, tests, or child-agent prompts. The agent must use scoped secret resolution/injection only for the verification or execution process at hand.
