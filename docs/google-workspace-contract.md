...
`auth.bindings.permissions.repair --dry-run` binds its effect digest to the protected-root identity, the ordered trusted-parent snapshots, and every exact target snapshot `(device, inode, type, uid, gid, link count, mode)`. Confirmation recomputes that plan and then reopens all targets with no-follow semantics before the first mode change. It changes only process-UID/GID-owned contained directories to `0700` and process-UID/GID-owned single-link regular files to `0600`, verifies both descriptors and names after the change, and is idempotent. The sole non-sticky shared exception is the complete observed Forge chain, without mode normalization: `root 02777 -> .local 02775 -> state 02775 -> openclaw 02775 -> google-workspace 02770`. The first four nodes are never repair targets; the protected root and credential files at `0660` are repairable. Outside that exact named chain, only process-UID/GID-owned exact `01777` sticky parents with a private containment boundary are accepted. Partial chains, generic `02777`/`02775` trust, and arbitrary sticky/setgid modes are rejected.
