...
`auth.bindings.status` validates the registry, referenced authentication-file presence, bundle shape, and identity consistency without exposing filesystem paths or credential bytes. Filesystem mode, UID, GID, ownership, symlink, and link-count metadata are deliberately ignored for authentication readiness. The former permission check/repair commands are retired.

Binding transactions retain alias/schema validation, bounded reads, locking, revisions, atomic replacement, backups, rollback, package-tree exclusions, and redaction. New artifacts may use private creation defaults, but runtime does not reject or mutate existing artifacts based on filesystem permission metadata.
