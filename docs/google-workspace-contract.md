...
`auth.bindings.permissions.repair --dry-run` binds its effect digest to the protected-root identity, the ordered trusted-parent snapshots, and every exact target snapshot `(device, inode, type, uid, link count, mode)`. Confirmation recomputes that plan and then reopens all targets with no-follow semantics before the first mode change. It changes only owned contained directories to `0700` and owned single-link regular files to `0600`, verifies the result, and is idempotent. The exact owned Forge legacy shape—`02777` trusted `workspace` ancestor, `02770` protected directories, and `0660` credential files—is repairable; the ancestor is never a target.
