...
`auth.login` is a one-shot Desktop-app flow that must run on the installing agent's own managed desktop. Before invoking it, start or inspect that agent's OpenClaw-managed browser and pass its literal loopback CDP/DevTools URL as `managedBrowserDevtoolsUrl`; the Harness may also read it from `GOOGLE_WORKSPACE_MANAGED_BROWSER_DEVTOOLS_URL` or `OPENCLAW_BROWSER_CDP_URL`, with explicit input taking precedence. Invoke login with `--account <alias> --transfer-root <private-dir> --output-path <relative-credential-file> --body '{"clientPath":"relative-installed-client.json","profiles":["workspace-max"],"managedBrowserDevtoolsUrl":"http://127.0.0.1:<port>","smokeTests":["gmail","calendar","drive"]}' --timeout-ms 600000 --json`; add `--overwrite` only when merging a new alias into an existing private bundle. The installed-client input and existing output must be regular, non-symlink mode-0600 files beneath the transfer root. The receiver binds only `127.0.0.1` on an OS-assigned port, uses state and PKCE S256, opens consent in that agent's desktop browser, and stores the credential locally. Never copy another agent's credential bundle or relay authorization URLs, codes, tokens, or client secrets between agents. The result contains sanitized alias/identity/scope metadata plus `desktopLocal` and `browserMode`, never credential contents. Device flow, web-client JSON, public callbacks, service accounts, and domain-wide delegation are not supported.
...
`auth.bindings.permissions.repair --dry-run` binds its effect digest to the protected-root identity, expected UID/GID from the exact verified protected parent snapshot, ordered trusted-parent snapshots, and every exact target snapshot `(device, inode, type, uid, gid, link count, mode)`. Confirmation recomputes that plan and reopens all targets with no-follow semantics before the first metadata change. It changes only process-UID-owned contained directories to the verified store GID and `0700`, and process-UID-owned single-link regular files to that GID and `0600`, using descriptor-bound `fchown`/`fchmod`; this safely migrates v0.3.4 stores created with a process-local GID. The existing exact `/root/.local/state/openclaw/google-workspace` and immediate `/workspace/<private-root>` boundaries remain narrow exceptions. Unknown names, extra traversal, arbitrary deeper roots, symlinks, multi-link files, and foreign UIDs are rejected.

The bounded inventory is the root, `credentials/`, `backups/`, registry, lock, strict credential/backup names, and strict registry-temp/credential-tombstone names. It never recursively traverses content. The complete operation fails closed for unknown content, an untrusted or changed parent snapshot, lexical/resolved escape, symlink, unsupported type, foreign UID, file hardlink, stale digest, missing safe no-follow primitives, or metadata/postcondition failure. Every target is validated before mutation, and partial apply triggers best-effort UID/GID/mode rollback through the already-open exact inode descriptors; a retry requires the still-current preview. Repair never reads credential or registry bytes, changes revisions, resolves aliases, invokes a provider, creates/deletes/renames artifacts, deletes legacy sources, or changes provider data. An absent fresh-install root is a metadata-only no-op.

New binding artifacts use the UID/GID of the exact verified protected store descriptor, or the exact verified parent snapshot when creating that store. This rule is agent- and numeric-GID-independent and never falls back to process EGID or supplementary groups. Credential, registry, lock, backup, staging, and temporary artifacts are created no-follow relative to verified directory descriptors, explicitly `fchown`ed and mode-restricted before use, and checked by descriptor/name inode identity. Missing or denied descriptor ownership control, path replacement, links, incomplete writes, fsync failures, or rename failures fail closed with atomic cleanup and stable committed revision/backup state.
