...
## v0.11 practical causal extension

The v0.11 semantic lane adds `Cause` and `Effect` and the `motivated_by` and `affected` contracts. Rationale extraction is anchored at sentence or list-item boundaries and never begins at an arbitrary suffix. A Decision subject must contain a complete decision, recommendation, selection, or completed actor-bound policy-action proposition; its Cause must contain a complete proposition, including factual dependency-gating and `can block` language. Present factual policy extraction is closed to actor-bound English `prohibits`/`requires` and Korean `요구한다`; policy objects use the existing 256-character evidence bound, retain dotted identifiers, and cannot cross a real sentence boundary; Korean evidence is subject-object-predicate while every other assertion remains subject-predicate-object. All spans remain exact, distinct, and non-overlapping, and reporting, quotation, generic recommendation, negation, conditionality, or epistemic uncertainty is rejected. Deontic `may`/`must`/`should` and an embedded `planned` modifier do not negate an explicit completed governing action, while modality governing the action or Cause remains rejected. Dotted identifiers, hyphenated terms, commas, and Korean equivalents remain inside their governing spans. A deterministic closed key may suppress a repeated stopped Plugin migration/direction paraphrase only when actor, explicit version, and Plugin subject agree; the richer rationale-bearing proposal remains exact-provenance evidence and the suppression is diagnosed. No general similarity merge occurs. Fragmentary subjects, incomplete actors, negation, governing nonfactual modality, chronology, incomplete causes, and workflow status language are rejected with safe diagnostics. All candidates remain untrusted, exact-span grounded, freshly sealed, and human-reviewed; v0.10 privacy and safety contracts remain unchanged.
...
`ontology-validate`, `review-queue`, `cq-evaluate`, and `semantic-view` are read-only, namespace-bound, canonical-source-preserving commands. v0.9 adds content-addressed private Entity Proposals grounded in exact canonical claim provenance. Only fresh, explicitly human-reviewed approved proposals may bootstrap `Person`, `Project`, `Decision`, or `Event` assertion endpoints; canonical explicit semantic entities remain separate and higher trust. Extracted, alias, and identity candidates never merge or project automatically. Causality requires direct evidence and human approval, chronology is insufficient, time precision is preserved, and stale provenance is inert. Semantic-first views hide structural edges by default, label approved private proposal nodes, and attach canonical hydration locators to every edge. These paths make no model, network, MCP, live graph, or canonical write. v0.8 bundles migrate read-only and v0.7 inference overlays remain separate and opt-in.
...
Only direct regular non-symlink Markdown files matching `<root>/memory/*.md` are source inputs. This graph, every snapshot, query result, diff, and export batch is noncanonical, disposable, and fully rebuildable. No command writes canonical Markdown. `onboard` is the sole mutating path: it verifies the live schema, invokes direct Memory MCP (never Gateway), and may mutate only its owned namespace.
...
The exact source boundary is direct regular non-symlink `memory/*.md`, relative to the supplied root. Every root file is excluded, including `MEMORY.md`, `memory.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`, `AGENTS.md`, `ORGANIZATIONS.md`, `WORKFLOW.md`, `BOOTSTRAP.md`, `HEARTBEAT.md`, and `TOOLS.md`. Every nested path is excluded, including `memory/.evidence/**`, `memory/.registry/**`, and any other subdirectory. Direct Markdown symlinks, a symlinked `memory` directory, and direct non-regular Markdown entries fail closed. Nested and root symlinks are outside the scan and ignored. Sources use class `memory_claim` and canonical-memory authority/precedence. The source digest binds only the ordered direct-memory source metadata.

Core document and section generation is obsolete. Compatibility fields `core_documents` and `core_sections` remain empty and compact core counts remain zero. Snapshot validation accepts legacy owned core entity/relation shapes only so diff/onboard reconciliation can delete them as stale. Namespace ownership still prevents deletion of foreign entities; incident relations to stale owned core nodes are removed before those owned nodes.

## Claim syntax

The primary and only input is the OpenClaw writer format in dynamically named direct `memory/*.md` topic files. A claim section has a heading, paired ID and JSON HTML comments, and the five writer bullets:
