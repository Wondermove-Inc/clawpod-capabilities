...
def base(root):return ['--output-root',root,'--session','case']
def test_plan_and_desktop_plan_are_pure(tmp_path):
 ...
 assert p['capability_profile']=='full' and set(p['selected_capabilities'])=={'read_content','insert_content','update_content','read_comments','insert_comments','read_user_information'}
 guide=' '.join(p['token_issuance_guide']).lower();assert 'protected secret capture' in guide and 'does not pre-authorize' in guide and 'allowedroots' in guide
 assert 'exposed' in p['exposed_token_recovery'].lower() and 'rotate' in p['exposed_token_recovery'].lower()
 secret_step=next(x for x in d['steps'] if x.get('reason')=='secret_capture_required');assert 'never chat/files/screenshots/logs' in secret_step['instruction'] and 'revoke/rotate' in secret_step['instruction']
...
def test_path_traversal_missing_root_public_root_and_symlinks(tmp_path):
 missing=tmp_path/'missing';rc,o=run('onboard.start','--output-root',missing,'--session','x','--workspace','Acme');assert rc==2 and 'already exist' in o['error']['message']
 root=private(tmp_path);rc,o=run('onboard.start','--output-root',root,'--session','../escape','--workspace','Acme');assert rc==2 and 'bounded relative' in o['error']['message']
 public=tmp_path/'public';public.mkdir(mode=0o755);rc,o=run('onboard.start','--output-root',public,'--session','x','--workspace','Acme');assert rc==2 and 'private' in o['error']['message']
 target=private(tmp_path/'target');link=tmp_path/'link';link.symlink_to(target,True);rc,o=run('onboard.start','--output-root',link,'--session','x','--workspace','Acme');assert rc==2 and 'symlink' in o['error']['message']
 (root/'bad').symlink_to(target,True);rc,o=run('onboard.start','--output-root',root,'--session','bad','--workspace','Acme');assert rc==2 and 'real child' in o['error']['message']
 session=root/'filelink';session.mkdir(mode=0o700);victim=tmp_path/'victim';victim.write_text('{}');(session/'state.json').symlink_to(victim);rc,o=run('onboard.start','--output-root',root,'--session','filelink','--workspace','Acme');assert rc==2 and 'non-symlink' in o['error']['message']
...
def test_gateway_scalar_integer_transport_and_strict_parser(tmp_path):
 h=json.loads((P.parent/'harness.json').read_text());contracts=json.loads((P.parent/'command_contracts.json').read_text())
 expected={'pageSize','maxItems','maxPages','maxDepth','timeoutMs','retries','sessionTimeout','now','expectedRevision'};seen=set()
 for name,cmd in h['commands'].items():
  for arg in contracts['commands'][name].get('integerStringTransport',[]):
   seen.add(arg);assert cmd['inputSchema']['properties'][arg]['type']=='string';assert next(x for x in cmd['argMap'] if x['arg']==arg)['valueType']=='string'
 assert seen==expected
 root=private(tmp_path);proc=subprocess.run([sys.executable,str(P),'onboard.start',*map(str,base(root)),'--workspace','Acme','--now','not-an-integer'],text=True,capture_output=True);assert proc.returncode==2 and 'invalid int value' in proc.stderr
...
def test_wrong_workspace_and_contract_has_no_fixture_or_arbitrary_path(tmp_path):
 ...
 h=json.loads((P.parent/'harness.json').read_text());blob=json.dumps({k:v for k,v in h['commands'].items() if k.startswith('onboard.')});assert 'adapterFixture' not in blob and 'statePath' not in blob and 'stateRoot' not in blob;assert h['commands']['onboard.start']['inputSchema']['required']==['outputRoot','session','workspace'];assert h['commands']['onboard.desktop.task']['safetyClasses']==['readOnly']
