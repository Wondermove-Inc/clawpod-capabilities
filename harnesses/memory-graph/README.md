...
Version 0.10.2 fixes the trusted Gateway mapping for the optional `semantic-extractor-input` private full-output contract: `output` remains a validated relative-name string without a path role, while `outputRoot` is the approved output path. The root must already exist and the output must be a normalized relative `.json` path beneath it. The Harness writes the complete canonical extractor page plus one newline atomically with mode `0600`, a 1 MiB hard limit, directory-FD traversal, symlink rejection, and parent-replacement detection. Stdout contains only the relative path, exact byte count, SHA-256 digest, and schema version. Supplying neither argument preserves the v0.10 full-stdout response. Canonical Markdown is never written.
