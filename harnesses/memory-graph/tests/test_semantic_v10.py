...
class SemanticV10(unittest.TestCase):
 ...
 def test_extractor_uses_exact_canonical_claim_prose_and_redacts_secrets(self):
  self.assertEqual(self.claim['claim_text'],'Mina is the reviewed participant in the non-live fixture.')
  self.assertTrue(self.claim['claim_text'])
  import importlib.util
  spec=importlib.util.spec_from_file_location('semantic_v10_claim_prose',P/'semantic_v10.py'); module=importlib.util.module_from_spec(spec); spec.loader.exec_module(module)
  core_spec=importlib.util.spec_from_file_location('memory_graph_claim_prose',P/'memory_graph.py'); core=importlib.util.module_from_spec(core_spec); core_spec.loader.exec_module(core)
  api={'error':ValueError,'inspect':core.inspect_workspace,'plan':core.build_plan,'namespace':core.namespace_for}
  plan,claims=module.fresh(api,self.t,'test-agent','test-workspace'); claims=copy.deepcopy(claims)
  claims['entity_person_mina']['claim']='password=abcdefghijklmnop'
  with mock.patch.object(module,'fresh',return_value=(plan,claims)):
   secret=next(row for row in module.extractor_input(self.t,'test-agent','test-workspace',api)['claims'] if row['claim_id']=='entity_person_mina')
  self.assertEqual(secret['claim_text'],'[REDACTED]'); self.assertNotIn('abcdefghijklmnop',json.dumps(secret))
  claims['entity_person_mina']['claim']=123
  with mock.patch.object(module,'fresh',return_value=(plan,claims)), self.assertRaises(TypeError): module.extractor_input(self.t,'test-agent','test-workspace',api)
 ...
 def test_extractor_pagination_is_stable_bounded_and_complete(self):
  ...
  drift=copy.deepcopy(pages); drift[-1]['page']['total']+=1; drift[-1]['bundle_hash']=module.sha({k:v for k,v in drift[-1].items() if k!='bundle_hash'})
  with self.assertRaises(ValueError): module.assemble_extractor_pages(drift,{'error':ValueError})
  if len(pages)>1:
 ...
 def test_temporal_dst_fold_is_explicit_and_nonexistent_wall_time_rejected(self):
  import importlib.util
  spec=importlib.util.spec_from_file_location('semantic_v10',P/'semantic_v10.py'); module=importlib.util.module_from_spec(spec); spec.loader.exec_module(module)
  first=module.normalized_time({'start':'2026-11-01T01:30:00-04:00','end':None,'timezone':'America/New_York','time_unknown':False})
  second=module.normalized_time({'start':'2026-11-01T01:30:00-05:00','end':None,'timezone':'America/New_York','time_unknown':False})
 ...
 def test_reconcile_rejects_malicious_shapes_duplicates_and_foreign_collisions(self):
  snap={'schema_version':'memory-graph-semantic-snapshot/v1','namespace':self.input['namespace'],'source_snapshot_hash':'a'*64,'source_digest':'b'*64,'entities':[{'semantic_id':'owned-e','entity_id':'person:a'}],'assertions':[],'candidates':[],'revoked':[],'quarantine':[],'inference_overlays':[]}; snap['snapshot_hash']=hashlib.sha256(json.dumps(snap,sort_keys=True,separators=(',',':')).encode()).hexdigest(); self.write('s.json',snap)
  cases=[
   ({'schema_version':'wrong','entities':[],'relations':[]},'invalid_memory_mcp_schema'),
   ({'schema_version':'memory-mcp/v1','entities':['not-an-object'],'relations':[]},'invalid_memory_mcp_schema'),
   ({'schema_version':'memory-mcp/v1','entities':[{'semantic_id':'x'},{'semantic_id':'x'}],'relations':[]},'duplicate_backend_semantic_id'),
   ({'schema_version':'memory-mcp/v1','entities':[{'semantic_id':'x','namespace':snap['namespace'],'semantic_owner':'other'}],'relations':[]},'ambiguous_semantic_ownership'),
   ({'schema_version':'memory-mcp/v1','entities':[{'semantic_id':'owned-e','namespace':'other','semantic_owner':'other'}],'relations':[]},'foreign_semantic_id_collision')]
  for current,code in cases:
   self.write('c.json',current); self.assertEqual(self.cli('semantic-reconcile','--input','s.json','--current','c.json',code=2)['error']['code'],code)
 ...
 def test_html_graph_dataset_escape_offline_deterministic_and_immutable(self):
  ...
  self.assertEqual(len({n['id'] for n in graph['nodes']}),3); self.assertTrue(all(re.fullmatch(r'node-[a-f0-9]{16}',n['id']) for n in graph['nodes']))
  self.assertEqual(len({e['id'] for e in graph['edges']}),2); self.assertTrue(all(re.fullmatch(r'edge-[a-f0-9]{16}',e['id']) for e in graph['edges'])); self.assertEqual(sum(e['status']=='inert' for e in graph['edges']),1)
  self.assertEqual(len({x['cluster'] for x in graph['nodes']+graph['edges']}),2); self.assertNotIn('review_reason',json.dumps(graph)); self.assertTrue(all('claim' not in x for x in graph['nodes']+graph['edges']))
  self.assertTrue(all(len(n['label'])<=512 for n in graph['nodes'])); self.assertIn('provenance',json.dumps([n['detail'] for n in graph['nodes']]))
  self.assertNotIn('</script><img',text); self.assertNotRegex(text,r'(?:src|href)=["\'](?:https?:)?//'); self.assertNotIn('http://',text); self.assertNotIn('https://',text); self.assertTrue(out['offline']); self.assertIn('Content-Security-Policy',text); self.assertIn("default-src 'none'",text); self.assertIn("connect-src 'none'",text); self.assertIn('name="referrer" content="no-referrer"',text); self.assertEqual(out['interactions'],['pan','zoom','node_drag','node_details','edge_details','keyboard_activation']); self.assertIn('<svg id="stage"',text); self.assertIn('edge-label',text); self.assertIn('UNAPPROVED / INERT',text); self.assertIn('marker-end',text); self.assertIn('relation-filter',text); self.assertIn('aria-live="polite"',text); self.assertIn("role:'button'",text); self.assertIn('nodeDrag',text)
 ...
 def test_semantic_query_is_bounded_and_returns_hydration_locators(self):
  ...
  self.assertEqual(len(module.semantic_query(snap,'person:a',{'error':ValueError},depth=1,direction='in')['entities']),1)
  self.assertEqual(len(module.semantic_query(snap,'project:b',{'error':ValueError},depth=1,direction='in')['entities']),2)
  self.assertEqual(len(module.semantic_query(snap,'project:b',{'error':ValueError},depth=1,direction='out')['entities']),1)
  isolated=copy.deepcopy(snap); isolated['assertions']=[]; isolated['snapshot_hash']=module.sha({k:v for k,v in isolated.items() if k!='snapshot_hash'})
  isolated_out=module.semantic_query(isolated,'person:a',{'error':ValueError}); self.assertEqual(len(isolated_out['entities']),1); self.assertFalse(isolated_out['assertions'])
  dangling=copy.deepcopy(snap); dangling['assertions'][0]['object']['entity_id']='project:missing'; dangling['snapshot_hash']=module.sha({k:v for k,v in dangling.items() if k!='snapshot_hash'})
  with self.assertRaises(ValueError): module.semantic_query(dangling,'person:a',{'error':ValueError})
  with self.assertRaises(ValueError): module.semantic_query(snap,'person:missing',{'error':ValueError})
  with self.assertRaises(ValueError): module.semantic_query(snap,'person:a',{'error':ValueError},direction='sideways')
 ...
 def test_private_state_json_write_is_atomic_mode_0600_and_repeatable(self):
  ...
  outside=Path(tempfile.mkdtemp()); linked=self.t/'linked-parent'; linked.symlink_to(outside,target_is_directory=True)
  with self.assertRaises(OSError): module.atomic_write_json(linked/'escape.json',value)
 ...
 def test_release_inventory_is_deterministic_complete_and_inert(self):
  ...
  out=json.loads(one); self.assertEqual(out['version'],'0.11.1'); self.assertIn('rollback',out); self.assertIn('update',out)
