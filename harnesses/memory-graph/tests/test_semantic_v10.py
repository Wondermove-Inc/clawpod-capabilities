...
class SemanticV10(unittest.TestCase):
 ...
 def test_chronology_cause_rejected(self):
  self.bundle['proposals'][-1]['payload']['predicate']='caused'; self.write('bundle.json',self.bundle); v=self.validated(); self.assertIn('chronology_only_cause',{x['reason_code'] for x in v['quarantine']})
