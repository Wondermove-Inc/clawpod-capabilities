...
def _legacy_export_html(snapshot,output,api,include_candidates=False):
 snapshot_entities_and_assertions(snapshot,api)
...
def export_html(bundle,output,api,include_candidates=False):
 """Render either a sealed inert candidate bundle or an approved snapshot."""
 schema=bundle.get("schema_version") if isinstance(bundle,dict) else None
 if schema=="memory-graph-validated-proposals/v1":
  if bundle.get("validated_hash")!=sha({k:v for k,v in bundle.items() if k!="validated_hash"}): fail(api,"invalid_validated_bundle","validated bundle is malformed or tampered")
  if include_candidates: fail(api,"invalid_export_mode","validated bundles are already wholly inert; include-candidates is snapshot-only")
  if not isinstance(bundle.get("entity_proposals"),list) or not isinstance(bundle.get("assertion_proposals"),list): fail(api,"invalid_validated_bundle","candidate collections must be arrays")
  records=bundle["entity_proposals"]+bundle["assertion_proposals"]
  if any(not isinstance(x,dict) for x in records): fail(api,"invalid_validated_bundle","candidate records must be objects")
  if any(x.get("lifecycle")!="candidate" for x in records): fail(api,"lifecycle_mislabeling","validated proposals must retain the validator's candidate lifecycle")
  mode="UNAPPROVED / INERT"; entities=bundle["entity_proposals"]; assertions=bundle["assertion_proposals"]
  quarantine=bundle.get("quarantine",[])
 elif schema==SCHEMA_SNAPSHOT:
  snapshot_entities_and_assertions(bundle,api)
  if any("label" in x and x.get("label") not in {"approved/private","approved/explicit"} for x in bundle.get("entities",[])+bundle.get("assertions",[])): fail(api,"lifecycle_mislabeling","approved projection records cannot carry a non-approved label")
  mode="APPROVED"; entities=bundle.get("entities",[]); assertions=bundle.get("assertions",[]); quarantine=bundle.get("quarantine",[])
  if include_candidates:
   candidates=bundle.get("candidates",[])
   if not isinstance(candidates,list): fail(api,"invalid_semantic_snapshot","snapshot candidates must be an array")
   if any("lifecycle" in x and x.get("lifecycle") not in {"candidate","tentative"} for x in candidates): fail(api,"lifecycle_mislabeling","snapshot candidate lane contains a non-candidate lifecycle")
   entities=entities+[x for x in candidates if x.get("kind")=="entity"]
   assertions=assertions+[x for x in candidates if x.get("kind")=="assertion"]
 else: fail(api,"unsupported_export_input","export input must be a validated candidate bundle or approved semantic snapshot")
 if len(entities)>500 or len(assertions)>1000: fail(api,"semantic_visualization_too_large","visualization is bounded to 500 nodes and 1000 edges")
 def payload(x): return x.get("payload",x)
 def endpoint(x): return x.get("entity_id","") if isinstance(x,dict) else ""
 def provenance(value):
  if not isinstance(value,dict): return {}
  path=value.get("path"); path=canonical_path(path)
  if path is None or SECRET.search(path): locator="[REDACTED]"
  else: locator="memory/source-"+sha({"namespace":str(bundle.get("namespace",""))[:256],"path":path})[:12]+".md"
  start=value.get("line_start"); end=value.get("line_end")
  return {"path":locator,"line_start":start if isinstance(start,int) and not isinstance(start,bool) and start>0 else None,"line_end":end if isinstance(end,int) and not isinstance(end,bool) and end>0 else None}
 namespace=str(bundle.get("namespace",""))[:256]
 def opaque(prefix,value): return prefix+"-"+sha({"namespace":namespace,"value":str(value)})[:16]
 raw_node_ids=[payload(x).get("entity_id") for x in entities if isinstance(x,dict)]
 if None in raw_node_ids or "" in raw_node_ids or len(set(raw_node_ids))!=len(entities): fail(api,"invalid_visualization_entity","entities require unique stable IDs")
 node_ids={raw:opaque("node",raw) for raw in raw_node_ids}
 claim_ids=sorted({str(x.get("claim_id","")) for x in entities+assertions})
 clusters={raw:opaque("cluster",raw) for raw in claim_ids}
 nodes=[]
 for index,x in enumerate(sorted(entities,key=lambda item:(payload(item).get("type","Unknown"),payload(item).get("entity_id",""))),1):
  p=payload(x); inert=schema=="memory-graph-validated-proposals/v1" or x.get("kind")=="entity"
  status="inert" if inert else "approved"; source=x.get("source",{})
  typ=p.get("type","Unknown") if p.get("type") in TYPES else "Unknown"
  nodes.append({"id":node_ids[p["entity_id"]],"type":typ,"cluster":clusters[str(x.get("claim_id",""))],"status":status,"label":f"{typ} {index}","detail":{"status":"UNAPPROVED / INERT" if inert else "APPROVED","type":typ,"provenance":provenance(source)}})
 edges=[]
 for index,x in enumerate(sorted(assertions,key=lambda item:str(item.get("semantic_id") or item.get("proposal_id") or sha(item))),1):
  p=payload(x); source=endpoint(p.get("subject")); target=endpoint(p.get("object"))
  if source not in node_ids or target not in node_ids: fail(api,"dangling_semantic_assertion","visualization assertion has a missing endpoint")
  inert=schema=="memory-graph-validated-proposals/v1" or x.get("kind")=="assertion"; status="inert" if inert else "approved"
  relation=p.get("predicate") if p.get("predicate") in PREDICATES else "unknown"
  edges.append({"id":opaque("edge",x.get("semantic_id") or x.get("proposal_id") or index),"source":node_ids[source],"target":node_ids[target],"relation":relation,"cluster":clusters[str(x.get("claim_id",""))],"status":status,"detail":{"status":"UNAPPROVED / INERT" if inert else "APPROVED","relation":relation,"subject_type":p.get("subject",{}).get("type") if isinstance(p.get("subject"),dict) else None,"object_type":p.get("object",{}).get("type") if isinstance(p.get("object"),dict) else None,"provenance":provenance(x.get("source",{}))}})
 graph={"schema_version":"memory-graph-html-dataset/v2","export_status":mode,"nodes":sorted(nodes,key=lambda x:x["id"]),"edges":sorted(edges,key=lambda x:x["id"])}
 data=html_json(graph)
 doc='''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; connect-src 'none'; img-src 'none'; media-src 'none'; font-src 'none'; object-src 'none'; frame-src 'none'; worker-src 'none'; base-uri 'none'; form-action 'none'"><meta name="referrer" content="no-referrer"><title>Offline Memory Graph</title><style>
html,body{height:100%;margin:0;background:#09111f;color:#e9f0fb;font:14px system-ui}body{display:grid;grid-template-rows:auto 1fr}.bar{padding:10px;background:#101b2d;border-bottom:1px solid #33435e}.status{font-weight:900;font-size:18px;padding:6px 12px;border:2px solid currentColor;border-radius:6px;margin-right:10px}.status.inert{color:#ffbf47}.status.approved{color:#65e6a8}input,select,button{padding:7px;margin:3px;background:#17253a;color:#eef;border:1px solid #50627f;border-radius:5px}main{min-height:0;display:grid;grid-template-columns:1fr 340px}svg{width:100%;height:100%;touch-action:none;background:radial-gradient(circle,#26344b 1px,transparent 1px);background-size:24px 24px}aside{overflow:auto;padding:14px;background:#101827;border-left:1px solid #33435e;white-space:pre-wrap}.edge{fill:none;stroke:#8fa3c2;stroke-width:2;cursor:pointer}.edge.inert{stroke:#ffbf47;stroke-dasharray:7 5}.edge-label{fill:#d6e1f3;font-size:11px}.node{stroke:#fff;stroke-width:2;cursor:grab}.node.inert{stroke:#ffbf47;stroke-dasharray:5 3}.Person{fill:#5b8def}.Project{fill:#46b5a7}.Decision{fill:#a878e8}.Cause{fill:#e06c75}.Effect{fill:#e5a84b}.Event{fill:#6fcf6a}.node-label{fill:#fff;font-size:12px;pointer-events:none}
</style></head><body><header class="bar"><span id="status" class="status"></span><input id="search" aria-label="Search" placeholder="Search"><select id="entity-filter" aria-label="Entity type"><option value="">All entity types</option><option>Person</option><option>Project</option><option>Decision</option><option>Cause</option><option>Effect</option><option>Event</option></select><select id="relation-filter" aria-label="Relation"><option value="">All relations</option></select><select id="claim-filter" aria-label="Evidence cluster"><option value="">All evidence clusters</option></select><select id="status-filter" aria-label="Approval status"><option value="">All statuses</option><option value="approved">APPROVED</option><option value="inert">UNAPPROVED / INERT</option></select><button id="reset">Reset pan/zoom</button></header><main><svg id="stage" role="img" aria-label="Interactive SVG node-link memory graph"><defs><marker id="arrow-approved" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto"><path d="M0,0 L10,3.5 L0,7 Z" fill="#8fa3c2"/></marker><marker id="arrow-inert" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto"><path d="M0,0 L10,3.5 L0,7 Z" fill="#ffbf47"/></marker></defs><g id="viewport"></g></svg><aside id="details" aria-live="polite">Activate a node or arrowed relation edge for bounded provenance details.</aside></main><script id="graph-data" type="application/json">'''+data+'''</script><script>
const G=JSON.parse(document.getElementById('graph-data').textContent),S=document.getElementById('stage'),V=document.getElementById('viewport'),D=document.getElementById('details'),NS=S.namespaceURI,Q=document.getElementById('search'),TF=document.getElementById('entity-filter'),RF=document.getElementById('relation-filter'),CF=document.getElementById('claim-filter'),SF=document.getElementById('status-filter');const ST=document.getElementById('status');ST.textContent=G.export_status;ST.className='status '+(G.export_status==='APPROVED'?'approved':'inert');[...new Set(G.edges.map(x=>x.relation))].sort().forEach(x=>RF.add(new Option(x,x)));[...new Set([...G.nodes,...G.edges].map(x=>x.cluster))].sort().forEach(x=>CF.add(new Option(x,x)));let z=1,tx=0,ty=0,pan=null,nodeDrag=null,pos={};function E(n,a){const e=document.createElementNS(NS,n);Object.entries(a).forEach(([k,v])=>e.setAttribute(k,v));return e}function activate(e,fn){if(e.key==='Enter'||e.key===' '){e.preventDefault();fn()}}function match(x){return(!Q.value||JSON.stringify(x).toLowerCase().includes(Q.value.toLowerCase()))&&(!CF.value||x.cluster===CF.value)&&(!SF.value||x.status===SF.value)}function transform(){V.setAttribute('transform',`translate(${tx} ${ty}) scale(${z})`)}function updateNode(id){const p=pos[id],g=V.querySelector(`g[data-id="${id}"]`);if(g){const c=g.querySelector('circle'),t=g.querySelector('text');c.setAttribute('cx',p.x);c.setAttribute('cy',p.y);t.setAttribute('x',p.x+25);t.setAttribute('y',p.y+4)}V.querySelectorAll(`path[data-source="${id}"],path[data-target="${id}"]`).forEach(path=>{const a=pos[path.dataset.source],b=pos[path.dataset.target];path.setAttribute('d',`M${a.x} ${a.y} L${b.x} ${b.y}`);const label=path.nextElementSibling;label.setAttribute('x',(a.x+b.x)/2);label.setAttribute('y',(a.y+b.y)/2-5)})}function draw(){V.textContent='';const nodes=G.nodes.filter(n=>match(n)&&(!TF.value||n.type===TF.value)),ids=new Set(nodes.map(n=>n.id)),w=Math.max(S.clientWidth,640),h=Math.max(S.clientHeight,520);nodes.forEach((n,i)=>{if(!pos[n.id]){const a=2*Math.PI*i/Math.max(nodes.length,1)-Math.PI/2;pos[n.id]={x:w/2+Math.cos(a)*Math.min(w,h)*.34,y:h/2+Math.sin(a)*Math.min(w,h)*.34}}});G.edges.filter(e=>ids.has(e.source)&&ids.has(e.target)&&match(e)&&(!RF.value||e.relation===RF.value)).forEach(e=>{const a=pos[e.source],b=pos[e.target],show=()=>D.textContent=JSON.stringify(e.detail,null,2),p=E('path',{d:`M${a.x} ${a.y} L${b.x} ${b.y}`,class:'edge '+e.status,'marker-end':`url(#arrow-${e.status})`,tabindex:0,role:'button','aria-label':`${e.status} relation ${e.relation}`,'data-source':e.source,'data-target':e.target});p.onclick=show;p.onkeydown=event=>activate(event,show);V.append(p);const t=E('text',{x:(a.x+b.x)/2,y:(a.y+b.y)/2-5,class:'edge-label'});t.textContent=e.relation;V.append(t)});nodes.forEach(n=>{const p=pos[n.id],show=()=>D.textContent=JSON.stringify(n.detail,null,2),g=E('g',{tabindex:0,role:'button','aria-label':`${n.status} ${n.type} ${n.label}`,'data-id':n.id}),c=E('circle',{cx:p.x,cy:p.y,r:20,class:`node ${n.type} ${n.status}`}),t=E('text',{x:p.x+25,y:p.y+4,class:'node-label'});t.textContent=n.label;g.append(c,t);g.onclick=show;g.onkeydown=event=>activate(event,show);g.onpointerdown=e=>{e.stopPropagation();g.setPointerCapture(e.pointerId);nodeDrag={id:n.id,x:e.clientX,y:e.clientY,target:g}};g.onpointermove=e=>{if(nodeDrag&&nodeDrag.id===n.id){pos[n.id].x+=(e.clientX-nodeDrag.x)/z;pos[n.id].y+=(e.clientY-nodeDrag.y)/z;nodeDrag.x=e.clientX;nodeDrag.y=e.clientY;updateNode(n.id)}};g.onpointerup=g.onpointercancel=e=>{if(nodeDrag&&nodeDrag.target===g){if(g.hasPointerCapture(e.pointerId))g.releasePointerCapture(e.pointerId);nodeDrag=null}};V.append(g)});transform()}S.onwheel=e=>{e.preventDefault();z=Math.max(.25,Math.min(4,z*(e.deltaY<0?1.1:.9)));transform()};S.onpointerdown=e=>pan={x:e.clientX-tx,y:e.clientY-ty};S.onpointermove=e=>{if(pan){tx=e.clientX-pan.x;ty=e.clientY-pan.y;transform()}};S.onpointerup=S.onpointerleave=()=>pan=null;document.getElementById('reset').onclick=()=>{z=1;tx=ty=0;pos={};draw()};[Q,TF,RF,CF,SF].forEach(x=>{x.oninput=draw;x.onchange=draw});addEventListener('resize',draw);draw();
</script></body></html>'''
 encoded=doc.encode("utf-8"); atomic_write(output,encoded)
 return {"schema_version":"memory-graph-semantic-html/v2","input_kind":"validated_candidates" if mode.startswith("UNAPPROVED") else "approved_snapshot","display_status":mode,"path":str(output),"sha256":hashlib.sha256(encoded).hexdigest(),"offline":True,"network_requests":0,"node_count":len(nodes),"edge_count":len(edges),"candidate_lane_included":mode.startswith("UNAPPROVED") or include_candidates,"quarantine_count":len(quarantine),"quarantine_projected":False,"filters":["search","entity_type","relation","evidence_cluster","status"],"interactions":["pan","zoom","node_drag","node_details","edge_details","keyboard_activation"],"labels":[mode]}
