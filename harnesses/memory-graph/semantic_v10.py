...
def approve(validated,manifest,api):
 if not closed(manifest,{"schema_version","namespace","reviewer_id","reviewed_at","decisions"}) or manifest["schema_version"]!=SCHEMA_APPROVAL: fail(api,"invalid_approval_manifest","closed approval manifest required")
...
def export_html(snapshot,output):
 ...
 for e in snapshot.get("entities",[]):
  ...
  nodes.append({"id":e.get("entity_id") or e["semantic_id"],"semantic_id":e["semantic_id"],"type":e.get("type","Unknown"),"cluster":e.get("claim_id","unknown"),"label":e.get("name") or e.get("entity_id") or e["semantic_id"],"trust":trust,"status":"approved","detail":e})
 edges=[]
 for a in snapshot.get("assertions",[]):
  edges.append({"id":a["semantic_id"],"source":endpoint_id(a.get("subject")),"target":endpoint_id(a.get("object")),"relation":a.get("predicate",""),"cluster":a.get("claim_id","unknown"),"status":"approved","trust":"approved assertion","detail":a})
 for c in snapshot.get("candidates",[]):
  p=c.get("payload",{}); kind=c.get("kind")
  if kind=="entity": nodes.append({"id":p.get("entity_id") or c["proposal_id"],"semantic_id":c["proposal_id"],"type":p.get("type","Unknown"),"cluster":c.get("claim_id","unknown"),"label":p.get("entity_id") or c["proposal_id"],"trust":"candidate/inert","status":"candidate","detail":c})
  elif kind=="assertion": edges.append({"id":c["proposal_id"],"source":endpoint_id(p.get("subject")),"target":endpoint_id(p.get("object")),"relation":p.get("predicate",""),"cluster":c.get("claim_id","unknown"),"status":"candidate","trust":"candidate/inert","detail":c})
