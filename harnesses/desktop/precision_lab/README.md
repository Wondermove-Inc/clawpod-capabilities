...
When invoking an installed copy outside the source worktree, bind provenance explicitly:

```sh
python3 /workspace/harnesses/desktop/precision_lab/precision_benchmark.py \
  --source-repo /path/to/source-worktree --candidate-commit "$(git -C /path/to/source-worktree rev-parse HEAD)"
```

The benchmark verifies that the requested candidate resolves to the source worktree's current `HEAD` and records the canonical source path, candidate, `origin/main`, merge base, and provenance mode in `baseline.json`. It never infers source provenance from an installed parent directory.
