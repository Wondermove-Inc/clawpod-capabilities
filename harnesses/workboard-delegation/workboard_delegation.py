...
VERSION="0.3.1"; SCHEMA=1
MAX_SNAPSHOT_BYTES=65536; MAX_PLAN_BYTES=65536; MAX_TEXT=12000; MAX_LABELS=32; MAX_STDOUT_BYTES=1900
...
def make_plan(a):
 ...
 candidate={"ok":True,"schemaVersion":SCHEMA,"command":"plan","data":{"preview":result},"error":None,"performed":False,"effects":[]}
 if len(stable(candidate).encode())>MAX_STDOUT_BYTES: raise HarnessError("plan_output_too_large",f"Planned output exceeds the {MAX_STDOUT_BYTES}-byte Gateway preview budget; shorten delegation text")
 return result
...
def main(argv=None):
 ...
 try:
  ...
  if command=="status":data={"name":"workboard-delegation","title":"Workboard Delegation","version":VERSION,"pure":True,"gatewayCalls":False,"mutates":False,"maxSnapshotBytes":MAX_SNAPSHOT_BYTES}
  ...
  print(stable(envelope(True,command,data=data)));return 0
 except HarnessError as e:
  print(stable(envelope(False,command,error={"code":e.code,"message":e.message,"details":redact(e.details),"performed":False,"retrySafe":False,"verification":"failed"})));return 2
 except SystemExit:raise
 except Exception as e:
  print(stable(envelope(False,command,error={"code":"internal_error","message":"Unexpected harness failure","details":redact(str(e)),"performed":False,"retrySafe":False,"verification":"failed"})));return 3
