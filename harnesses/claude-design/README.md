...
Deterministic browser-first guardrails for Claude Design onboarding, auth/browser readiness, exact short/long prompt input, projects, sharing, the layout quality gate, link-first handoff, opt-in native exports, design systems, templates, code sync, destinations, and administration. Version 0.4.3 (calibrated on a real export) adds `projects.qa.layout` and `projects.link.verify`; the surface is now 66 commands.
