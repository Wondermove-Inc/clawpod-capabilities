...
class Backend:
    ...
    def _raw_request(self,method,path,body=None,headers=None,idempotency=None,deadline=None):
        ...
        for i in range(attempts):
            ...
            if remaining is not None and remaining<=0: raise BackendError("plan_timeout","lifecycle deadline exceeded",False)
            try:
                req=urllib.request.Request(self.base+path,data=raw,headers=hs,method=method)
                with self.opener.open(req,timeout=min(self.timeout,remaining) if remaining is not None else self.timeout) as rsp:
                ...
                if retry and i+1<attempts:
                    delay=min(.05*(2**i),.2)
                    self._deadline_sleep(delay,deadline); continue
                ...
                if method=="GET" and i+1<attempts:
                    delay=min(.05*(2**i),.2)
                    self._deadline_sleep(delay,deadline); continue
                raise BackendError("timeout","backend request timed out",retry_safe=method=="GET")
            except (socket.timeout,TimeoutError):
                if method=="GET" and i+1<attempts: self._deadline_sleep(min(.05*(2**i),.2),deadline); continue
                raise BackendError("timeout","backend request timed out",retry_safe=method=="GET")
    def _deadline_sleep(self,delay,deadline):
        if deadline is not None and deadline-time.monotonic()<=delay: raise BackendError("plan_timeout","lifecycle deadline exceeded",False)
        time.sleep(delay)
        if deadline is not None and time.monotonic()>=deadline: raise BackendError("plan_timeout","lifecycle deadline exceeded",False)
    @staticmethod
    def _check_deadline(deadline):
        if deadline is not None and time.monotonic()>=deadline: raise BackendError("plan_timeout","lifecycle deadline exceeded",False)
    def login_from_env(self,deadline=None):
        ...
        try:
            ...
            key_data=self._raw_request('GET','/api/auth/public-key',deadline=deadline)
            ...
            self._check_deadline(deadline)
            self._raw_request('POST','/api/auth/login',body={'email':email,'password':ciphertext,'rememberMe':False},deadline=deadline)
            self._check_deadline(deadline)
            self.authenticated=True
        except BackendError: raise
        except Exception:
            raise BackendError('auth_contract_error','RSA-OAEP login setup failed',False)
    def request(self,method,path,body=None,headers=None,idempotency=None,authenticated=True,deadline=None):
        if authenticated and not self.authenticated: self.login_from_env(deadline=deadline)
        return self._raw_request(method,path,body,headers,idempotency,deadline=deadline)
