...
Desktop OAuth coverage includes loopback success, denied consent, missing refresh token, state mismatch, duplicate callback rejection, timeout, bind failure, token/identity/scope errors, malformed or absent client files, atomic output, overwrite refusal, alias collision and merge, traversal rejection and linked-file acceptance, secret redaction, URL encoding, PKCE S256 correctness, repeat invocation, and browser fallback. The command inventory contains 167 commands (147 provider commands plus 20 local auth, binding, onboarding, and detached-login commands).
...
- exhaustive 167-command contract coverage, including 147 provider HTTPS operations, closed command-specific schemas, required resource identifiers, and non-empty exact scope declarations
...
## v0.3.8 local validation

- `python3 -m pytest -q harnesses/google-workspace/tests`: 131 passed, 167 subtests passed.
- Detached OAuth tests cover typed lifecycle schemas, the 10-second Gateway bound, fail-fast legacy login, malformed handles, bounded recovery, and the static no-nested-control-plane invariant.
