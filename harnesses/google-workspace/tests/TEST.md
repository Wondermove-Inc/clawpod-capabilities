...
- protected-store GID inheritance across credential, registry, lock, backup, and temporary artifacts, including a synthetic `/workspace` store at GID 1000 with a different process default GID; descriptor-relative no-follow creation, explicit ownership/mode application, stable inode/link/containment verification, legacy-source preservation, and clean retry after injected chown/write/replace failures
