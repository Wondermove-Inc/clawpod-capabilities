...
class AdversarialContract(unittest.TestCase):
 ...
 def test_resumable_upload_and_checksum_download(self):
  src=Path(self.d.name)/'in';src.write_bytes(b'hello');self.mock([{'headers':{'Location':'https://upload.invalid/session'}},{'body':{'id':'f'}}]);out,code=run('drive.files.upload',{'account':'a','transferRoot':self.d.name,'inputPath':'in','params':{'uploadType':'resumable'},'body':{'name':'x'}});self.assertEqual(code,0,out)
