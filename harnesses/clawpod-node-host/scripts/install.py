...
source = Path(__file__).resolve().parents[1] / "clawpod_node_host.py"
target = Path(a.bin_dir).expanduser().resolve() / ("clawpod-node-host.cmd" if os.name == "nt" else "clawpod-node-host")
