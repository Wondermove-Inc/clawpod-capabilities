...
commands={
'source.fetch':({'url':'string','outputRoot':'string','snapshot':'string','timeout':'integer','maxBytes':'integer'},['url'],False,[('outputRoot','--output-root','output'),('snapshot','--snapshot','output')]),
'source.batch':({'inputRoot':'string','manifest':'string','outputRoot':'string','output':'string','timeout':'integer','maxBytes':'integer'},['inputRoot','manifest'],False,[('inputRoot','--input-root','input'),('manifest','--manifest','input'),('outputRoot','--output-root','output'),('output','--output','output')]),
'source.import':({'inputRoot':'string','capture':'string','sourceUrl':'string','mediaType':'string','outputRoot':'string','output':'string','maxBytes':'integer'},['inputRoot','capture'],False,[('inputRoot','--input-root','input'),('capture','--capture','input'),('outputRoot','--output-root','output'),('output','--output','output')]),
'bundle.build':({'inputRoot':'string','sources':'string','claims':'string','outputRoot':'string','output':'string'},['inputRoot','sources','claims','outputRoot','output'],True,[('inputRoot','--input-root','input'),('sources','--sources','input'),('claims','--claims','input'),('outputRoot','--output-root','output'),('output','--output','output')]),
'bundle.validate':({'inputRoot':'string','bundle':'string'},['inputRoot','bundle'],False,[('inputRoot','--input-root','input'),('bundle','--bundle','input')]),
...
for name,(props,required,mutation,paths) in commands.items():
 ...
 pathmap={x:(f,r) for x,f,r in paths}; flags={'url':'--url','timeout':'--timeout','maxBytes':'--max-bytes','sourceUrl':'--source-url','mediaType':'--media-type'}; amap=[]
 for k,v in props.items():
  flag,role=pathmap.get(k,(flags.get(k),None)); ent={'arg':k,'type':'option','flag':flag,'valueType':'path' if role else ('number' if v=='integer' else 'string'),'optional':k not in required}
