...
commands=list(READ)+list(MUTATE)+['auth.login.start','auth.login.status','auth.login.cancel']
props={k:{'type':'string'} for k in ['host','expectedAccount','repo','state','number','runId','title','body','head','base','review','mergeMethod','tag','file','endpoint','idempotencyKey','jobId','jobsDir','confirm']}
...
for k,v in props.items():
 ...
 item={'arg':k,'type':'booleanFlag' if v['type']=='boolean' else 'option','flag':flag,'valueType':'boolean' if v['type']=='boolean' else ('integer' if v['type']=='number' else ('path' if k in {'file','jobsDir'} else 'string')),'optional':True}
...
for c in commands:
 ...
 if c=='auth.login.start':safety=['secretUse','humanAccountAction','externalSideEffect']
 if c=='auth.login.cancel':safety=['writeSafe']
 ...
 contracts['commands'][c]={'backend':'gh','mutation':c in MUTATE or c.startswith('auth.login'),'required':req,'inputSchema':ci,'outputSchema':co}
