...
props={k:{'type':'string'} for k in ['host','expectedAccount','repo','source','sourceBranch','remoteBranch','expectedRemoteSha','visibility','description','homepage','state','number','runId','title','body','head','base','review','mergeMethod','tag','file','endpoint','confirm']}
...
for k,v in props.items():
 ...
 item={'arg':k,'type':'booleanFlag' if v['type']=='boolean' else 'option','flag':flag,'valueType':'boolean' if v['type']=='boolean' else ('integer' if v['type']=='number' else ('path' if k in {'file','source'} else 'string')),'optional':True}
 if k in {'file','source'}:item['pathRole']='input'
 argmap.append(item)
...
manifest={'schemaVersion':1,'kind':'openclaw.harness.v1','name':'github','title':'GitHub','description':'Guarded typed GitHub operations through the real gh CLI with stable JSON and bounded execution.','version':'0.3.1','entrypoint':'./github.py','packageRoot':'.','execution':{'cwd':'.','timeoutMs':30000,'requiresJson':True},'whenToUse':['Operate GitHub repositories, issues, pull requests, workflow runs, releases, local repository bootstrap and guarded branch push, or bounded API reads'],'capabilities':['gh-cli','guarded-mutations','verified-release-body-update','verified-repository-bootstrap','verified-branch-push','stable-json'],'authModel':{'type':'pre-authenticated-gh-cli','storesSecrets':False,'requiresHumanAccount':True},'commands':{}}
contracts={'schemaVersion':1,'commands':{}}
def camel(name):
 parts=name.split('_')
 return parts[0]+''.join(part[:1].upper()+part[1:] for part in parts[1:])
for c in commands:
 req=[camel(x) for x in REQUIRED.get(c,[])]
